...
- `chatgpt-only`: every row must use the OpenAI OAuth account `codex-chatgpt-subscription`; no Anthropic/Alibaba account may appear. The five-probe verifier expects Sol for main, Terra-medium for stale foreign IDs, Terra-medium/medium for trusted-header compact and verified Agent SDK traffic, and Sol/high for the built-in Plan composite. The published catalog and observed target/upstream routes must contain no Luna.
...
For `chatgpt-only`, require a host/WSL Codex auth SHA-256 match, an active schedulable OpenAI OAuth account, a GPT-only `/v1/models` catalog without Luna, empty fallbacks, clean profile check-only output, correlated Agent SDK rows on Terra-medium/medium, trusted-header compact on Terra-medium/medium, and a structurally verified Plan row on Sol/high. Treat standalone `claude -p` as a main CLI request on Claude Code 2.1.219; require the `agent-sdk/...` marker before applying the child route and never broaden the proxy matcher to all `(external, cli)` or generic `sdk-cli` traffic. Read `plan-agent-premium-routing.md` before changing the Plan signature.
