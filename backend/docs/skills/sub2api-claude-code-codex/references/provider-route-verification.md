...
- `chatgpt-only`: every row must use the OpenAI OAuth account `codex-chatgpt-subscription`; no Anthropic/Alibaba account may appear. The six-probe verifier expects Sol for main, Luna/max for stale foreign IDs, trusted-header compact, and verified Agent SDK traffic, Sol/high for the built-in Plan composite, and routes an explicit legacy Terra/max request to Luna/max. The published catalog must include Luna and hide Terra-medium; observed target/upstream routes must contain no Terra-medium row.
