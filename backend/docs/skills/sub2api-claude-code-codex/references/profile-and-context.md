...
Claude Code default Haiku model: gpt-5.6-terra-high while native Spark is quota-limited, with normal model_fallbacks to gpt-5.6-sol-medium
Claude Code subagent overrides: gpt-5.6-terra-high with effort high while native Spark is quota-limited, with empty-output fallback to gpt-5.6-sol-medium
...
Persistence warning: never run `docker compose down -v` on this profile unless the user explicitly asks to wipe Headroom memory/embeddings and sub2api state. Use `docker compose up -d --build` or recreate individual services without deleting volumes.
...
- Current override: `model: gpt-5.6-terra-high` plus `effort: high`. Keep this while native `gpt-5.3-codex-spark` is quota-limited; do not switch all subagents to Sol/max unless the user explicitly wants maximum quality over quota safety. Configure normal messages `model_fallbacks` so Terra-high falls back to `gpt-5.6-sol-medium` after empty-output/unavailable failures.
