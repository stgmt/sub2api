...
Claude Code picker category defaults: opus/fable/sonnet/haiku passthrough aliases; do not hide Qwen, Terra, or GPT under Opus/Fable/Sonnet/Haiku labels
...
- Current User env pairing: `ANTHROPIC_SMALL_FAST_MODEL=gpt-5.3-codex-spark`, picker category defaults `ANTHROPIC_DEFAULT_OPUS_MODEL=opus`, `ANTHROPIC_DEFAULT_FABLE_MODEL=fable`, `ANTHROPIC_DEFAULT_SONNET_MODEL=sonnet`, `ANTHROPIC_DEFAULT_HAIKU_MODEL=haiku`, and `CLAUDE_CODE_SUBAGENT_MODEL=gpt-5.6-terra-medium`. This keeps compact/small-fast on Spark fallback logic, keeps the picker honest, and prevents delegated paths from inheriting parent Sol/max. Normal message fallbacks stay empty unless the user explicitly requests one.
