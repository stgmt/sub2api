...
- Alibaba role bench on 2026-07-22 through Headroom/sub2api used exact-JSON, compact-sentinel, and subagent-review prompts. Spark baseline was still strongest for small-fast/compact (`avg_s=3.8`, `avg_score=100`). Among Alibaba models, `qwen3.8-max-preview` was the only all-pass quality pick (`avg_s=14.4`, `avg_score=100`), `qwen3.7-plus` passed subagent-review faster but missed compact sentinels (`avg_s=10.9`, `avg_score=88`), `qwen3.6-flash` was fastest (`avg_s=6.3`) but lost compact sentinels and one severity, and `glm-5.2` / `deepseek-v4-pro` are not safe defaults because visible output can be empty or low-fidelity when reasoning consumes the response budget.
...
- Keep `ANTHROPIC_SMALL_FAST_MODEL=gpt-5.3-codex-spark` for compact/small-fast work, with compact-only fallback from `gpt-5.3-codex-spark` to `gpt-5.6-luna`, unless the user explicitly chooses an Alibaba small-fast profile after reviewing the bench. Spark is officially 128k and text-only, so long/image-bearing compacts require the Luna fallback. Qwen3.8 Max is the safest Alibaba compact candidate; Qwen3.6 Flash is only suitable for short low-risk Haiku-style calls, not compact handoffs.
