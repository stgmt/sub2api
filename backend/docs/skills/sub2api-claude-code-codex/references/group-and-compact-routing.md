...
- Under `chatgpt-only`, `plan_mapped_model=gpt-5.6-sol` and `plan_reasoning_effort=high` override only a built-in Plan request accepted by the system-plus-UA-plus-tools classifier. Generic SDK children remain `gpt-5.6-luna/xhigh`; compact precedence remains `gpt-5.6-luna/high`. See `plan-agent-premium-routing.md` for anchors, session stickiness, injection resistance, and live proof.
