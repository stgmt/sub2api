...
- Under `chatgpt-only` v5, `plan_mapped_model=gpt-5.6-sol` and `plan_reasoning_effort=high` override only a built-in Plan request accepted by the system-plus-UA-plus-tools classifier. Verified Agent SDK children and trusted-header compact use `gpt-5.6-luna/max`; model and automatic fallbacks are empty, and Luna is the explicit non-Plan target. See `plan-agent-premium-routing.md` for anchors, session stickiness, injection resistance, and live proof.
- The chatgpt-only compatibility redirect is request-time and server-side. `exact_model_mappings` controls the target model, while `exact_model_reasoning_efforts` controls `output_config.effort` for the requested or mapped exact ID. The v5 profile sets Terra, Terra-medium, Luna, and the stale delegated aliases to `max`; therefore an already-running Claude Code process sending `medium` or `xhigh` is recorded as Luna/max without a client restart. Plan is classified earlier and remains Sol/high.
