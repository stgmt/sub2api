...
- The configured `qwen3.8-max-preview -> gpt-5.6-sol` entry is a candidate, not an unconditional provider fallback. sub2api consumes it only for automatic compact/SDK-CLI routes after the exact terminal Token Plan quota body or when the scheduler has no available Qwen account. A selected healthy Qwen stays on Alibaba; direct interactive Qwen, transient 429/5xx, context/auth/model errors, and failures after stream bytes remain on the original route.
