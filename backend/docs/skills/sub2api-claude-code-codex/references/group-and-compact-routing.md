...
- Under `chatgpt-only` v4, `plan_mapped_model=gpt-5.6-sol` and `plan_reasoning_effort=high` override only a built-in Plan request accepted by the system-plus-UA-plus-tools classifier. Verified Agent SDK children and trusted-header compact use `gpt-5.6-terra-medium/medium`; model and automatic fallbacks are empty, and Luna is not a target. See `plan-agent-premium-routing.md` for anchors, session stickiness, injection resistance, and live proof.
