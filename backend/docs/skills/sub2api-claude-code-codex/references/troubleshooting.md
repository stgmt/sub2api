...
- The autostart guard for this class is deliberate: a transient Headroom `/health` failure must not run `docker compose up --force-recreate` over the entire profile. `ensure-sub2api-proxy-stack.ps1` first reads `GET /stats` and `proxy_inbound.active`; active traffic or an unproven running state yields `recovery_deferred`, while recreation is permitted only for zero active requests or a stopped/missing required container. A live `/health=200` after the incident proves recovery, not that the earlier stream was delivered.
