...
- Keep Headroom `--embedding-server` enabled in this image. The upstream `headroom-ai==0.31.0` wheel exposes the flag but omits `headroom.memory.adapters.watchdog`; the local Dockerfile patches in a Unix-socket watchdog plus socket embedder client. If logs show `No module named 'headroom.memory.adapters.watchdog'` or `Falling back to per-worker embedder`, the image is stale or was built without `patch-headroom-embedding-server.py`. Rebuild `headroom-sub2api:0.31.0` and recreate the `headroom` service, then verify logs show `Embedding server: ready.`
- Preserve Headroom embeddings/memory across recreates. `/root/.headroom` must be a persistent mount and should contain `ccr_store.db` after memory traffic. `/root/.cache/headroom` and `/root/.cache/huggingface` should also be persistent mounts so local tool/model/embedding caches survive image/container churn. Do not use `docker compose down -v` unless intentionally wiping memory and all service state.
