...
- If `/fast` appears enabled but speed is unchanged, trace both request and response rather than trusting the picker. The Anthropic beta header or `speed:"fast"` must produce `openai_messages.fast_mode_forwarded` with `forwarded=true` and final request `service_tier=priority`; check the raw Chat Completions fallback too. Then inspect `openai_messages.fast_mode_provider_confirmation`: only `provider_confirmed=true` proves the provider used Fast. New `usage_logs.service_tier` rows use the returned provider tier when available, so `default` means the Codex/OpenAI provider declined Fast.
