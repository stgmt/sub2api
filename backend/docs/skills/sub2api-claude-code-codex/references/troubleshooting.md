...
- First classify the requested model family. GPT/Codex names should route to the OpenAI/Codex account; `qwen*`, `glm-*`, and `deepseek-v4-pro` should route to the Alibaba Token Plan Anthropic-compatible account; `fable`, `opus`, `sonnet`, `haiku`, and `claude-*` should route only to a real Anthropic-compatible account.
...
- Do not fix Claude/Fable unknown-model errors by mapping them to GPT. With no real Anthropic-compatible account configured, a fast `404 model_not_found/no available accounts` with `fallback_used=false` is the correct transparent behavior.
...
If `/v1/models` lists `gpt-5.5-mini`, `gpt-5.6-*`, Qwen/Token Plan, or Claude/Fable names but requests fail:
...
- Current verified behavior on 2026-07-22: `/v1/models` through Headroom returned 34 explicit models; a tiny `gpt-5.6-terra-medium` request routed to the OpenAI/Codex account with `reasoning_effort=medium`, a tiny `qwen3.8-max-preview` request routed to the Alibaba Token Plan account with no `pricing not found` error, and `opus` failed fast with `platform=anthropic`, `model_not_found=true`, and `fallback_used=false` because no real Anthropic-compatible account was configured.
- Spark is officially 128k and text-only with separate research-preview rate limits. Use it for compact/small-fast, but do not expect it to carry a 300k+ Claude session or image-bearing compact without the Luna fallback.
