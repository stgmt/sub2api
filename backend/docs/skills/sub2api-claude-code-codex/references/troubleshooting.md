...
- Ensure User env and `~/.claude/settings.json` contain `MAX_THINKING_TOKENS=8000`, but do not treat it as the Codex reasoning control. For GPT-5.6, verify `reasoning_effort=max` in sub2api logs for max requests; `xhigh` is now only the legacy max fallback for older models.
...
- Do not add a global Agent-blocking hook as a routine fix. On this machine the accepted guard is advisory: `workflowSizeGuideline=small` plus `%USERPROFILE%\.claude\agents\general-purpose.md` pinned to `gpt-5.6-terra` with prompt guidance to stay under 10 sibling agents and avoid deep chains. Hard-blocking `PreToolUse` / `SubagentStart` / `SubagentStop` state-file hooks should be installed only after explicit user approval. Do not tell the user a built-in depth cap will prevent hundreds of spawned descendants; observed Claude Code runs contradicted that operationally.
