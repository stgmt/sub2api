...
- Current verified behavior on 2026-07-22 after the Qwen-high picker correction: direct sub2api and restarted Headroom `/v1/models` returned 23 explicit models, GPT/Codex plus Alibaba Token Plan only. A tiny `gpt-5.6-terra-medium` request routes to the OpenAI/Codex account with `reasoning_effort=medium`; `qwen3.8-max-preview`, `glm-5.2`, and `deepseek-v4-pro` route to the Alibaba Token Plan account with no `pricing not found` error.
