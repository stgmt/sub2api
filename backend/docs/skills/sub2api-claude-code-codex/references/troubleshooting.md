...
- The group candidate `qwen3.8-max-preview -> gpt-5.6-sol` is consumed only by automatic compact and `external,sdk-cli` routes. The first terminal failure may switch before response bytes; while the circuit is open, later automatic requests go straight to Sol/high without probing Qwen. Direct interactive Qwen, generic 429/5xx, auth/context/model errors, and mid-stream errors do not switch providers. Logs must show `alibaba_token_plan_quota_exhausted` and `claude_code.automatic_cross_provider_fallback`.
