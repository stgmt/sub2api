...
- Verify the Headroom patch from inside the container: `return JSONResponse(content=queued, status_code=202)` must be absent, sentinels `Claude Code session key patch`, `Claude Code no-202 overlap patch`, `active-stream refcount patch`, and `overlap wait patch` must be present, and `HEADROOM_MID_TURN_STREAM_WAIT_MS` should normally be `600000`. If an overlap is real, logs should show `event=mid_turn_overlap_wait` and then `event=mid_turn_overlap_wait_done`, not a `202` response to Claude Code.
