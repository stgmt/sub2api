...
- Headroom post-terminal variant (F45): if sub2api records HTTP 200 with real input/output tokens while Headroom logs `Exception in ASGI application` and `UnboundLocalError ... 'tags'` from `streaming.py`, the provider did not fail. The nested generator must construct `final_tags = dict(tags or {})`, pass `tags=final_tags`, and guard `_finalize_stream_response` with a non-fatal `except Exception` after stream output. Run `test_headroom_claude_code_streaming_patch.py` and its mutation driver, then rebuild/recreate only Headroom. Live proof requires ordered `message_start`, content, `message_stop`, clean EOF, a successful `claude --print`, no new ASGI/finalizer errors, and no monotonic `proxy_inbound.active` gap. Do not retry or recreate sub2api for this class.
- Headroom partial-output variant (F46): if a request produced text and then `Upstream request failed`, `Server error mid-response`, `incomplete chunked read`, or a missing terminal event, do not retry the original generation after visible output. Verify the running fork has `AnthropicCheckpointRelay`, `/health.runtime.claude_stream_recovery.enabled=true`, and host `Stop` plus `SubagentStop` entries for `claude-stream-recovery.mjs`. A recovered wire ends with `message_stop`, Headroom logs `event=claude_stream_checkpoint`, the ledger exposes one pending marker, and Claude's JSONL contains meta `Stop hook feedback` followed by another assistant request with the same `session_id`. An incomplete `tool_use` must never appear on the client wire; completed tools use the normal tool-result continuation and do not need the Stop hook. Reproduce the full client path with `scripts/prove-claude-stream-recovery.ps1`; require exit code `0`, two upstream requests, one unique session id, and both transcript markers `FIRST_PART` and `SECOND_PASS`. Do not fail the proof solely because `console_checkpoint_events=0`; structured logging may be routed away from Docker stdout.
...
- First distinguish a provider outage from a dead resolver. `curl /health=200` proves only Headroom-to-sub2api reachability. Run `wsl.exe -d Ubuntu-24.04 -- getent ahostsv4 chatgpt.com` and `wsl.exe -d Ubuntu-24.04 -- docker exec sub2api-codex getent ahostsv4 chatgpt.com`. The combination of missing WSL `/etc/resolv.conf`, Docker `NO EXTERNAL NAMESERVERS DEFINED`, and sub2api `lookup chatgpt.com on 127.0.0.11:53: server misbehaving` is a local DNS outage. Run `scripts/repair-wsl-dns.ps1 -ProfileDir <runtime> -RepairContainers`; do not wait out or enlarge an upstream hold without repairing DNS.
- The compose profile sets explicit `SUB2API_PRIMARY_DNS` and `SUB2API_FALLBACK_DNS` for both proxies. On a routed/VPN LAN, set the primary to the host's real LAN resolver so split/synthetic DNS remains consistent; keep a reachable public fallback. The repeating self-heal probes from inside `sub2api-codex` and patches `/etc/resolv.conf` in place, so no live Claude stream needs a container recreate.
- If the Claude JSONL says `incomplete chunked read` or `Server error mid-response` and `sub2api-codex` has a `StartedAt`/compose recreate within about one second, the failure is a local compose lifecycle cut, not proof of an upstream model error. Docker `restart=0` does not disprove this: `docker compose recreate` replaces the container without incrementing the restart counter. Correlate Claude JSONL timestamp, Headroom request log, sub2api `StartedAt`, and compose/self-heal logs before retrying the prompt.
- The autostart guard for this class is deliberate: a transient Headroom `/health` failure must not run `docker compose up --force-recreate` over the entire profile. `ensure-sub2api-proxy-stack.ps1` first reads `GET /stats` and `proxy_inbound.active`. That raw value includes the `/stats` request itself, so `raw_active=1` means observer-adjusted `active=0`; failing to subtract it makes every idle rollout unreachable. Active user traffic or an unproven running state yields `recovery_deferred`, while recreation is permitted only for adjusted zero activity or a stopped/missing required container. A live `/health=200` after the incident proves recovery, not that the earlier stream was delivered.
- The same watchdog safely applies a newly built Headroom tag when it detects image drift: it waits for `proxy_inbound.active=0` and recreates only `headroom` with `--no-deps --force-recreate`. During active or unknown traffic it records `headroom_image_rollout_deferred`; it never replaces `sub2api` just to deploy a Headroom image.
- If the self-heal log says `Could not translate Codex auth path into WSL` while a manual `wslpath` probe returns `/mnt/c/...`, check for the WSL localhost-proxy warning being mixed into native command output. The maintained script filters for the absolute POSIX result; do not fix this by deleting OAuth state or starting a second stack.
...
- If sub2api logs `stream ID ... INTERNAL_ERROR; received from peer`, `RST_STREAM`, `GOAWAY`, `unexpected EOF`, or `connection reset by peer` after OpenAI HTTP `200`, this is a transport-body failure, not a successful completion. The running fork records the read error, marks only that normalized upstream/proxy key H1-only, and lets the existing one-at-most pre-output retry use the H1 client pool. After visible text or `tool_use`, replay is prohibited; do not raise retry counts or globally disable H2. Confirm `openai_http2_reset_fallback_activated` or `openai_http2_body_fallback_activated`, then verify the next request and TTL H2 probe.
...
- If `/fast` appears enabled but speed is unchanged, trace both request and response rather than trusting the picker. The Anthropic beta header or `speed:"fast"` must produce `openai_messages.fast_mode_forwarded` with `forwarded=true` and final wire `service_tier=priority`, matching native Codex OAuth serialization. Check the raw Chat Completions fallback too. Then inspect `openai_messages.fast_mode_provider_confirmation`: only `provider_confirmed=true` with provider tier `priority` proves the requested Fast wire tier was honored. New `usage_logs.service_tier` rows use the returned provider tier when available, so `default` means the Codex/OpenAI provider declined or downgraded Fast.
- Native Codex uses WebSocket `wss://chatgpt.com/backend-api/codex/responses`, while Claude Code through Headroom enters as HTTP `/v1/messages`. The Claude/Codex compose profile enables the compatible local WS ingress at `/v1/responses` with `GATEWAY_OPENAI_WS_MODE_ROUTER_V2_ENABLED=true` and `GATEWAY_OPENAI_WS_INGRESS_MODE_DEFAULT=ctx_pool`; a direct native-style `response.create` probe must show handshake `101`, `response.created`, output events, and `response.completed`. This transport proof still does not prove Fast: the current OAuth upstream can return `auto`/`default` after a `priority` request.
- With a custom `ANTHROPIC_BASE_URL`, Claude Code may reject `/fast` locally before sending `/v1/messages`. The chatgpt-only profile uses the supported client identity `claude-opus-5`, plus both `CLAUDE_CODE_SKIP_FAST_MODE_NETWORK_ERRORS=1` and `CLAUDE_CODE_SKIP_FAST_MODE_ORG_CHECK=1`; the latter is required when `CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC=1` suppresses the availability request. sub2api maps ordinary `claude-opus-5` requests to Sol and a verified Fast request to its configured `fast_mapped_model` (Luna). This is not a provider override: require `provider_confirmed=true` in sub2api logs before calling the request genuinely Fast. Do not invent or configure `CLAUDE_CODE_FORCE_FAST_MODE`; it is not a supported client setting.
- Upstream boundary: OpenAI Codex issue [#30413](https://github.com/openai/codex/issues/30413) documents ChatGPT-auth requests sending `service_tier=priority` but completing with `service_tier=default`; [#32191](https://github.com/openai/codex/issues/32191) documents the same behavior for `codex exec` on GPT-5.6. OpenAI's [Fast-mode documentation](https://developers.openai.com/codex/speed) lists GPT-5.6, GPT-5.5, and GPT-5.4, but this live ChatGPT OAuth account rejects generic `gpt-5.6` with upstream 400 and returns `default` for accepted `gpt-5.6-sol`/Luna requests. Treat `provider_confirmed=false` as a real standard-tier downgrade, preserve the request/response evidence, and do not silently label it Fast. `gpt-5.6-luna` is the supported model-level low-latency fallback; a generic `gpt-5.5-fast` or guessed `gpt-5.6-fast` mapping is not a valid ChatGPT OAuth fallback.
...
- Check for the proven rate-limit/cooldown pattern. The key signature is `openai_messages.account_select_failed` with `error="no available accounts"` immediately before an access log `status_code=429` or legacy `status_code=503` on `/v1/messages`:
  ...
  ```
- For native Anthropic OAuth, find the upstream event before the no-account burst. `This request would exceed your account's rate limit` plus official Anthropic reset headers is a real subscription-window limit. Current sub2api behavior persists that reset, returns the exact model/reset message with `Retry-After`, admits the account automatically after the timestamp, and clears cooldown state on the first successful Anthropic response. The autonomous paired profile sets both `HEADROOM_UPSTREAM_429_MAX_WAIT_SECONDS=86400` and `HEADROOM_UPSTREAM_TRANSIENT_MAX_WAIT_SECONDS=86400`; the transient value is capped by the former in Headroom, so configuring only the transient value leaves an effective six-hour deadline. Headroom allows only one response-header probe per upstream URL, credential, and requested model. `stream:true` gets a held HTTP 200 Anthropic SSE stream with `ping` every 15 seconds. `stream:false` (native compact, fallback, and some background turns) gets a held HTTP 200 chunked `application/json` body; leading whitespace is the only legal keepalive before the recovered JSON. A Sonnet/Opus hold must leave GPT/Qwen cooldown at zero even though all models use one stable sub2api key; a successful buffered or streaming response must clear the matching stale model circuit. `HEADROOM_RETRY_MAX_ATTEMPTS=10` covers short failures; after that, configured `HEADROOM_UPSTREAM_RECOVERY_HOLD_STATUSES=429,502,503,504,529` and transport disconnects enter the matching wire-mode hold. A live success probe is insufficient: reproduce a controlled 429-to-200 in both modes, parse the buffered wire as JSON, prove cross-model isolation and successful-path cleanup, and require `holds_total` plus `recoveries_total` increments. Do not clear a future reset or enable cross-provider fallback inside `anthropic-only`; inspect `/health.runtime.upstream_recovery` and Headroom logs for `upstream_cooldown_hold_start`, `upstream_cooldown_probe_deferred`, and `upstream_cooldown_hold_recovered`.
...
- Before treating that failover error as a genuinely empty model response, inspect `output_text_done_bytes`, `content_part_done_text_bytes`, and `output_item_done_text_bytes`. Some Codex Responses streams omit every `response.output_text.delta` while carrying the complete paid answer in all three done snapshots and an empty terminal `response.output`. The bridge must reconcile those snapshots by `(output_index, content_index)`, emit only the missing suffix, and log `openai_messages.stream_terminal_text_recovered`; a same-account retry is both wasteful and unable to repair the conversion bug.
