...
RTK execution: pinned RTK 0.42.4 on Windows plus WSL, global Claude Code `PreToolUse(Bash)` through an MSYS-safe WSL hook, and shared host history mounted at `/root/.local/share/rtk` for Headroom metrics
...
- Keep Headroom, lean-ctx, and TokenSave service/MCP binaries inside Docker for this profile. RTK is the deliberate exception: Claude Code executes Bash on the Windows host, so container-only RTK cannot rewrite those commands. Install pinned RTK on Windows and in WSL with `scripts/install-claude-rtk.ps1`; keep exactly one global `PreToolUse(Bash)` WSL bridge with `MSYS2_ARG_CONV_EXCL='*'`; preserve the `cat`, `git diff`, `git show`, and `curl` accuracy exclusions; and bind the host RTK state to Headroom `/root/.local/share/rtk`. If `claude mcp list` shows stale host `headroom` or `tokensave` entries, remove them and re-add only Docker-backed Headroom.
...
- Windows and WSL both have pinned RTK, exactly one Claude Code Bash hook survives, the hook is MSYS-safe, a real fresh Claude Code Bash call creates a new RTK history row, accuracy-excluded commands remain raw, and Headroom `/root/.local/share/rtk` reports the same command/savings totals as host `rtk gain --format json`.
