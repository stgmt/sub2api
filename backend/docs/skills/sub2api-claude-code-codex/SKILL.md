...
- Native `/compact` recognition is a two-proxy contract. Headroom must preserve the exact compact message, add `x-sub2api-claude-compact: 1`, and skip output shaping; sub2api treats that header as authoritative and applies `compact_model_mapping`. For compact sessions containing image blocks, Spark's exact image-input HTTP 400 must activate the configured Luna fallback. Prove this with a real forked Claude Code `/compact`, Headroom's `headroom:claude_code_compact_prompt_preserved` marker, sub2api fallback logs, and `usage_logs`; a synthetic prompt alone is insufficient.
