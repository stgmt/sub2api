...
Assert-Contains $ensure 'HostKey C:/ProgramData/ssh/ssh_host_ed25519_key' "Windows OpenSSH must use a real ProgramData host-key path"
Assert-Contains $ensure 'AuthorizedKeysFile C:/ProgramData/ssh/administrators_authorized_keys' "Windows OpenSSH must use a real ProgramData authorized-key path"
Assert-Contains $ensure 'StrictModes no' "Bootstrap SSH must not reject the key due to offline ACL translation"
Assert-Contains $ensure 'Copy-Item -LiteralPath $publicKeyPath -Destination $administratorsAuthorizedPath -Force' "Offline SSH repair must refresh stale administrator key material"
Assert-Contains $ensure 'icacls.exe $administratorsAuthorizedPath /grant' "Offline SSH repair must repair stale authorized-key ACLs before overwrite"
Assert-Contains $ensure 'profileCandidates' "Offline SSH repair must discover an existing guest profile instead of trusting a stale username"
Assert-Contains $ensure 'requested_user' "Offline SSH repair must record requested and selected guest users"
Assert-Contains $ensure 'profileCandidates' "Offline SSH repair must validate the selected guest user against existing guest profiles"
