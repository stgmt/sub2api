...
Assert-Contains $sync 'ProviderSyncToken' "Grok sync must use the scoped provider-sync credential"
Assert-Contains $sync 'Sync-Sub2apiProviderAccount' "Grok sync must use the service-owned provider-sync API"
...
if ($sync -match '(?i)psql|INSERT\s+INTO\s+accounts|UPDATE\s+accounts|Restart-Sub2api|AdminPassword') {
  throw "Grok sync must not use SQL, restarts, or administrator credentials"
}
