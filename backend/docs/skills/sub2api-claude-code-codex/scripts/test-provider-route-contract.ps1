...
Assert-True ($anthropic.expected_provider -eq "anthropic") "Anthropic proof contract must name provider"
  ...
  Assert-True ($chatgpt.client_env.ANTHROPIC_DEFAULT_FABLE_MODEL -eq "gpt-5.6-terra-medium") "ChatGPT-only Fable picker slot must use Terra-medium"
  Assert-True ($chatgpt.client_env.ANTHROPIC_DEFAULT_SONNET_MODEL -eq "gpt-5.6-terra-medium") "ChatGPT-only Sonnet picker slot must use Terra-medium"
  ...
  Assert-True ($chatgpt.group.messages_dispatch_model_config.sdk_cli_mapped_model -eq "gpt-5.6-terra-medium") "ChatGPT-only SDK CLI must use Terra-medium"
  Assert-True ($chatgpt.group.messages_dispatch_model_config.sdk_cli_reasoning_effort -eq "medium") "ChatGPT-only SDK CLI must use medium"
  Assert-True ($chatgpt.group.messages_dispatch_model_config.sonnet_mapped_model -eq "gpt-5.6-terra-medium") "ChatGPT-only Sonnet dispatch must use Terra-medium"
  ...
  Assert-True ($chatgpt.group.messages_dispatch_model_config.exact_model_mappings.'qwen3.8-max-preview' -eq "gpt-5.6-terra-medium") "Stale Qwen IDs must fail closed to Terra-medium"
  Assert-True ($chatgpt.group.messages_dispatch_model_config.exact_model_mappings.'gpt-5.6-luna' -eq "gpt-5.6-terra-medium") "Stale Luna IDs must fail closed to Terra-medium"
  Assert-True ($chatgpt.group.messages_dispatch_model_config.exact_model_mappings.'gpt-5.6-terra' -eq "gpt-5.6-terra-medium") "Raw Terra IDs must fail closed to Terra-medium"
